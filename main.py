...
from config import initialize_system_csvs
...
def note_table(page: ft.Page = None):
    '''Renders note board panel configuration with lookup search engine filters'''
    ROOT = os.getcwd()
    event_data_list = NOTE_PATH # Đồng bộ với NOTE_PATH chính ở hàm main
    ...
    def save_edit_note(e):
        nonlocal editing_index
        if os.path.exists(NOTE_PATH) and editing_index is not None:
            try:
                df = pd.read_csv(NOTE_PATH, encoding='utf-8-sig')
                df.dropna(how='all', inplace=True)
...
def event_cards_view(page: ft.Page = None):
    '''Renders note board panel configuration with lookup search engine filters'''
    ROOT = os.getcwd()
    event_data_list = EVENT_PATH # Đồng bộ với NOTE_PATH chính ở hàm main
    ...
    def save_edit_note(e):
        nonlocal editing_index
        if os.path.exists(EVENT_PATH) and editing_index is not None:
            try:
                df = pd.read_csv(EVENT_PATH, encoding='utf-8-sig')
                df.dropna(how='all', inplace=True)
...
def academy_tier_selector(page: ft.Page = None):
                ...
                cards.append(
                    ft.Container(
                        ...
                        content=ft.Row([
                            ...
                            ft.Row([
                                ft.Container(content=ft.Text(f"RANK: {row.get('kda', '0.0')}", size=12, weight=ft.FontWeight.W_500), bgcolor=ft.Colors.GREY_900, padding=6, border_radius=5),
    ...
    def build_statistics_tab(tier_name):
        ...
        chart_sections = []
        if total_players == 0:
            chart_sections.append(fch.PieChartSection(100, title="No Data", color=ft.Colors.GREY_600, radius=50))
        else:
            if under_1 > 0:
                chart_sections.append(fch.PieChartSection(under_1, title=f"Below Average\n({under_1})", color=ft.Colors.RED_600, radius=50, title_style=ft.TextStyle(size=10, weight="bold")))
            if mid_1_15 > 0:
                chart_sections.append(fch.PieChartSection(mid_1_15, title=f"Medium\n({mid_1_15})", color=ft.Colors.ORANGE_600, radius=50, title_style=ft.TextStyle(size=10, weight="bold")))
            if high_15_2 > 0:
                chart_sections.append(fch.PieChartSection(high_15_2, title=f"Good\n({high_15_2})", color=ft.Colors.BLUE_600, radius=50, title_style=ft.TextStyle(size=10, weight="bold")))
            if pro_2_plus > 0:
                chart_sections.append(fch.PieChartSection(pro_2_plus, title=f"Elite\n({pro_2_plus})", color=ft.Colors.GREEN_600, radius=50, title_style=ft.TextStyle(size=10, weight="bold")))
            ...
            ft.Row([
                ft.Container(
                    content=ft.Column([
                        ...
                        ft.Container(content=pie_chart, height=200, alignment=ft.alignment.Alignment.CENTER)
                    ]),
...
def settings_view(page: ft.Page, _file_picker_ignored=None):
    '''Returns an English localized modular configuration setting view panel with Report Opening functionality (Sync)'''
    ROOT = os.getcwd()
    ...
    def get_report_files():
        try:
            ...
            return [ft.dropdown.Option(f) for f in files] if files else [ft.dropdown.Option("No reports available")]
        except Exception:
            return [ft.dropdown.Option("No reports available")]

    drop_report_select = ft.Dropdown(
        label="Select Report / Dataset to Open",
        options=get_report_files(),
        expand=True,
        text_size=14
    )

    # Hàm làm mới danh sách (Sync)
    def refresh_dropdown(e):
        drop_report_select.options = get_report_files()
        drop_report_select.update()
        page.show_dialog(ft.SnackBar(ft.Text("Available reports list refreshed!"), open=True))

    # Hàm mở file trực tiếp trên hệ điều hành (Sync)
    def trigger_open_file(e):
        selected_file = drop_report_select.value
        if not selected_file or selected_file == "No reports available":
            page.show_dialog(ft.SnackBar(ft.Text("Please select a valid report file first!"), bgcolor=ft.Colors.RED_700, open=True))
            return
        ...
        if not os.path.exists(file_path):
            page.show_dialog(
                ft.SnackBar(ft.Text("File does not exist on disk!"), bgcolor=ft.Colors.RED_700, open=True)
            )
            return
        ...
        try:
            ...
            page.show_dialog(
                ft.SnackBar(ft.Text(f"Opening {selected_file}..."), open=True)
            )
        except Exception as ex:
            page.show_dialog(
                ft.SnackBar(ft.Text(f"Could not open file: {str(ex)}"), bgcolor=ft.Colors.RED_700, open=True)
            )

    # Hàm chuyển đổi giao diện Sáng/Tối (Sync)
    def toggle_theme(e):
    ...
    return ft.Container(
        ...
        content=ft.ListView([
            ...
            ft.Text("Access System Reports", size=18, weight=ft.FontWeight.W_600),
            ft.Text("Select a report from the dropdown list to open and view its content directly.", size=13, color=ft.Colors.GREY_500),
            ft.Container(height=5),
            
            ft.Column([
                ft.Row([
                    drop_report_select,
                    ft.IconButton(icon=ft.Icons.REFRESH, on_click=refresh_dropdown, tooltip="Refresh file list")
                ]),
                ft.Container(height=5),
                ft.Row([
                    ft.ElevatedButton(
                        content=ft.Text("Open Report File"),
                        icon=ft.Icons.LAUNCH,
                        bgcolor=ft.Colors.BLUE_700,
                        color="white",
                        on_click=trigger_open_file 
                    )
...
def main(page: ft.Page):
    ...
    match_time = ft.TimePicker(open=False, value=time(1, 2), entry_mode=ft.TimePickerEntryMode.INPUT_ONLY, on_change=save_time)
    ...
    # match_time.open = False
    # page.overlay.append(match_time)
    page.update()
    ...
    def save_member(e):
        if os.path.exists(MEMBERS_PATH):
            try:
                df = pd.read_csv(MEMBERS_PATH, encoding='utf-8-sig')
            except Exception:
                df = pd.DataFrame(columns=['STT', 'Clan', 'ingame', 'media', 'role', 'note'])
        else:
            df = pd.DataFrame(columns=['STT', 'Clan', 'ingame', 'media', 'role', 'note'])
        ...
        new_data = pd.DataFrame({
            'STT': pd.array([next_stt], dtype='int64'),
            'Clan': [clan_input.value],
            'ingame': [ingame_input.value],
            'media': [media_input.value],
            'role': [role_input.value],
            'note': [note_input.value],
        })

        # Ép các cột text luôn là kiểu string/object để tránh xung đột dtype
        for col in ['Clan', 'ingame', 'media', 'role', 'note']:
        ...
        add_dialog.open = False
        page.update()
        if hasattr(table_view, 'custom_refresh'):
            table_view.custom_refresh()
    ...
    add_options_dialog = ft.AlertDialog(
        ...
        actions=[
            ft.TextButton("New Member", on_click=lambda e: page.show_dialog(add_dialog)),
            ft.TextButton("New Custom Match", on_click=lambda e: page.show_dialog(match_dialog)),
            ft.TextButton("New Event", on_click=lambda e: page.show_dialog(events_dialog)),
            ft.TextButton("New Note", on_click=lambda e: page.show_dialog(notes_dialog)),
        ],
    )
    ...
    event_view = event_cards_view(page)
    notes_view = note_table(page)  
    ...
    page.icon = "images/IMT_Logo.png"

if __name__ == "__main__":
    initialize_system_csvs()
    print(MEMBERS_PATH, MATCH_PATH, EVENT_PATH, NOTE_PATH)
